...
object WweSchedule {
    ...
    private val knownSmackdown = listOf(
        ...
        LocalDate.of(2026, 10, 9), LocalDate.of(2026, 10, 16), LocalDate.of(2026, 10, 30), LocalDate.of(2026, 11, 13), LocalDate.of(2026, 11, 20), LocalDate.of(2026, 12, 4)
    )
    ...
    private fun show(date: LocalDate, title: String, league: String, hourEt: Int): SportsEvent {
        val start = ZonedDateTime.of(date, java.time.LocalTime.of(hourEt, 0), ZoneId.of("America/New_York")).toInstant().toString()
        val state = if (System.currentTimeMillis() >= ZonedDateTime.parse(start).toInstant().toEpochMilli()) "in" else "pre"
        return SportsEvent("wwe:${title.lowercase().replace(" ", "-")}:$date", "wrestling", league, title, title, state, start, emptyList(), emptyList(), null, "WWE • ${if (title.contains("Raw")) "Netflix" else "USA Network"}", if (title.contains("Raw")) "Netflix" else "USA Network")
    }
}
