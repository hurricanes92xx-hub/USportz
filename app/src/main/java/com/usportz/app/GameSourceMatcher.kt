...
/** Fast, bounded event -> channel matcher with normalized teams, broadcaster confidence and hard exclusions. */
object GameSourceMatcher {
    ...
    fun rankMatches(event: SportsEvent, channels: List<SportsChannel>, limit: Int = 16): List<Match> {
        ...
        return candidates.values.asSequence().mapNotNull { channel ->
            ...
            val unrelated = eventTeams.isNotEmpty() && containsUnrelatedTeam(haystack, eventTeams)
            if (unrelated) return@mapNotNull null
            ...
            .sortedWith(compareByDescending<Match> { it.confidence }
    ...
    private fun confidence(score: Int, matchedTeams: Int, teamCount: Int, broadcast: Int, normalizedBroadcast: Int, leagueHit: Boolean): Int {
        var value = (score * 100 / 170).coerceIn(0, 100)
        if (teamCount >= 2 && matchedTeams >= 2) value += 12
        else if (matchedTeams == 1) value += 4
    ...
    private fun containsUnrelatedTeam(text: String, eventTeams: List<String>): Boolean {
        ...
        return known.any { knownTeam ->
            ...
            canonicalKnown.length >= 5 && normalized.contains(canonicalKnown) &&
                eventTeams.none { it == canonicalKnown || it.contains(canonicalKnown) || canonicalKnown.contains(it) }
        }
    }
}

object SportsEventNormalizerText {
    fun channelText(channel: SportsChannel): String =
        listOf(channel.name, channel.group, channel.tvgName, channel.tvgId, channel.category, channel.provider)
            .filter { it.isNotBlank() }.joinToString(" ")
}

/** Small high-signal lexicon used only for hard exclusion; it is intentionally bounded. */
object SportsTeamLexicon {
    private val teams = setOf(
        ...
        "Syracuse", "Virginia", "Virginia Tech", "West Virginia", "Tennessee", "South Carolina",
        "Miami Dolphins", "Buffalo Bills", "New England Patriots", "New York Jets", "Baltimore Ravens",
        "Cincinnati Bengals", "Cleveland Browns", "Pittsburgh Steelers", "Jacksonville Jaguars",
        "Houston Texans", "Indianapolis Colts", "Tennessee Titans", "Kansas City Chiefs", "Denver Broncos",
        "Las Vegas Raiders", "Los Angeles Chargers", "Dallas Cowboys", "Philadelphia Eagles",
        "New York Giants", "Washington Commanders", "Green Bay Packers", "Chicago Bears",
        "Detroit Lions", "Minnesota Vikings", "Tampa Bay Buccaneers", "Atlanta Falcons",
        "Carolina Panthers", "New Orleans Saints", "Los Angeles Rams", "San Francisco 49ers",
