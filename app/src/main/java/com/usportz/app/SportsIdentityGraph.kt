...
object CanonicalChannelIdentity {
    ...
    private fun normalize(v: String?): String = v.orEmpty().lowercase(Locale.US).replace("&", " and ").replace(Regex("\\b(channel|network|hd|fhd|uhd|4k|east|west|central)\\b"), " ").replace(Regex("[^a-z0-9]+"), " ").trim()
...
/** Persistent EPG and event -> broadcast -> stream graph. */
class SportsIdentityGraphStore(context: Context) : SQLiteOpenHelper(context, "usportz_sports_graph.db", null, 1) {
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE epg_programs (id TEXT PRIMARY KEY, channel_id TEXT NOT NULL, title TEXT NOT NULL, start_ms INTEGER NOT NULL, end_ms INTEGER NOT NULL, description TEXT)")
    ...
    fun upsertChannel(channel: SportsChannel, network: String = "") {
        writableDatabase.execSQL("INSERT OR REPLACE INTO epg_channels(id,canonical_id,name,callsign,tvg_id,tvg_name,network,last_seen) VALUES(?,?,?,?,?,?,?,?)", arrayOf(channel.id, CanonicalChannelIdentity.key(channel), channel.name, CanonicalChannelIdentity.callsign(channel.tvgName.ifBlank { channel.name }), channel.tvgId, channel.tvgName, network, System.currentTimeMillis()))
    }
    ...
    fun replacePrograms(programs: List<EpgProgram>) {
        ...
        try {
            programs.forEach { p ->
                val id = "${p.channelId}:${p.startMs}:${p.title.hashCode()}"
                db.execSQL("INSERT OR REPLACE INTO epg_programs(id,channel_id,title,start_ms,end_ms,description) VALUES(?,?,?,?,?,?)", arrayOf(id, p.channelId, p.title, p.startMs, p.endMs, p.description))
            }
    ...
    fun connectStreams(event: SportsEvent, channels: List<SportsChannel>, limit: Int = 16): List<EventStreamEdge> = connectEvent(event, channels, limit).map { edge ->
        val c = channels.firstOrNull { it.id == edge.channelId }
        EventStreamEdge(event.id, edge.channelId, c?.url.orEmpty(), edge.score, c?.url?.substringAfterLast('.', "").orEmpty())
