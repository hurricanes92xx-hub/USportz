...
object SportsChannelBridge {
    ...
    /** Compatibility handshake: try player_api first, then probe get.php for portals that omit player_api. */
    fun validateXtream(server: String, user: String, pass: String): Boolean {
        ...
        val apiUrl = "$server/player_api.php?$query"
        val apiResult = runCatching { request(apiUrl, 5_000) }.getOrNull()
        if (!apiResult.isNullOrBlank()) {
            val json = runCatching { JSONObject(apiResult) }.getOrNull()
            if (json != null) {
                val info = json.optJSONObject("user_info")
                val auth = info?.optInt("auth", -1) ?: -1
                val status = clean(info?.optString("status"))
                if (auth == 1 || status.equals("Active", true) || status.equals("Enabled", true)) return true
                if (auth == 0 || status.equals("Invalid", true) || status.equals("Expired", true) || status.equals("Disabled", true) || status.equals("Banned", true)) return false
            } else if (apiResult.contains("#EXTM3U", true)) return true
        }
        // Some compatible panels expose playlist generation but a broken/blocked player_api endpoint.
        val playlistUrl = "$server/get.php?$query&type=m3u_plus&output=ts"
        return runCatching {
            val conn = URL(playlistUrl).openConnection() as HttpURLConnection
            try {
                conn.connectTimeout = 4_000; conn.readTimeout = 6_000; conn.instanceFollowRedirects = true; conn.requestMethod = "GET"
                conn.setRequestProperty("Accept", "application/x-mpegURL, audio/x-mpegurl, text/plain, */*"); conn.setRequestProperty("User-Agent", "USportz/1.3")
                ...
                val buffer = ByteArray(32 * 1024); val read = stream.read(buffer); stream.close()
                if (read <= 0) false else String(buffer, 0, read).contains("#EXTM3U", true) || String(buffer, 0, read).contains("#EXTINF", true)
    ...
    private fun request(url: String, timeout: Int): String {
        ...
        return try { conn.connectTimeout = 4_000; conn.readTimeout = timeout; conn.instanceFollowRedirects = true; conn.requestMethod = "GET"; conn.setRequestProperty("Accept", "application/json"); conn.setRequestProperty("Accept-Encoding", "gzip"); conn.setRequestProperty("User-Agent", "USportz/1.3"); if (conn.responseCode !in 200..299) return ""; val stream = if (conn.contentEncoding.equals("gzip", true)) GZIPInputStream(conn.inputStream) else conn.inputStream; stream.bufferedReader().use { it.readText().take(128_000) } } finally { conn.disconnect() }
    }
    ...
    private fun fetchAndParse(source: String): List<SportsChannel> { val conn = URL(source).openConnection() as HttpURLConnection; conn.connectTimeout = 6_000; conn.readTimeout = 60_000; conn.instanceFollowRedirects = true; conn.requestMethod = "GET"; conn.setRequestProperty("Accept", "application/x-mpegURL, audio/x-mpegurl, text/plain, */*"); conn.setRequestProperty("Accept-Encoding", "gzip"); conn.setRequestProperty("User-Agent", "USportz/1.3"); return try { val code = conn.responseCode; if (code !in 200..299) throw IllegalStateException("HTTP $code"); val raw = conn.inputStream; val stream = if (conn.contentEncoding.equals("gzip", true)) GZIPInputStream(raw) else raw; stream.bufferedReader().use(::parse) } finally { conn.disconnect() } }
