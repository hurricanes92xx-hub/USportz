...
object SportsChannelBridge {
    ...
    private fun isAuthenticatedResponse(body: String): Boolean {
        ...
        return authValue == 1 || authValue == true || authValue.toString().equals("1", true) || status.equals("Active", true) || status.equals("Enabled", true) || status.equals("Trial", true)
    }
    ...
    private fun probeM3u(url: String): Boolean {
        ...
        return try {
            conn.connectTimeout = 4_500; conn.readTimeout = 7_000; conn.instanceFollowRedirects = true; conn.requestMethod = "GET"
            conn.setRequestProperty("Accept", "application/x-mpegURL, audio/x-mpegurl, text/plain, */*"); conn.setRequestProperty("Accept-Encoding", "gzip"); conn.setRequestProperty("User-Agent", "USPortz/1.9")
            if (conn.responseCode !in 200..299) return false
            val text = openDecoded(conn).bufferedReader().use { reader -> val buffer = CharArray(32 * 1024); val read = reader.read(buffer); if (read > 0) String(buffer, 0, read) else "" }
    ...
    private fun normalizedServerCandidates(server: String): List<String> {
        val raw = server.trim().trimEnd('/'); if (raw.isBlank()) return emptyList()
        ...
        val scheme = uri.scheme?.lowercase() ?: return emptyList(); val authority = uri.rawAuthority?.takeIf { it.isNotBlank() } ?: return emptyList(); val path = uri.path.orEmpty().trimEnd('/')
        val basePath = when { path.endsWith("/player_api.php", true) -> path.dropLast("/player_api.php".length).trimEnd('/'); path.endsWith("/panel_api.php", true) -> path.dropLast("/panel_api.php".length).trimEnd('/'); path.endsWith("/get.php", true) -> path.dropLast("/get.php".length).trimEnd('/'); path.endsWith("/xmltv.php", true) -> path.dropLast("/xmltv.php".length).trimEnd('/'); else -> path }
        val root = "$scheme://$authority${basePath.ifBlank { "" }}"; val alternate = "${if (scheme == "https") "http" else "https"}://$authority${basePath.ifBlank { "" }}"; return listOf(root, alternate).distinct()
    }
    ...
    private fun parseCategoryNames(text: String): Map<String, String> {
        ...
        runCatching {
            ...
            if (root.startsWith("[")) {
                ...
                for (i in 0 until array.length()) { val item = array.optJSONObject(i) ?: continue; val id = item.optString("category_id").ifBlank { item.optString("id") }; val name = item.optString("category_name").ifBlank { item.optString("name") }; if (id.isNotBlank() && name.isNotBlank()) out[id] = name }
            } else {
                val obj = JSONObject(root); val array = obj.optJSONArray("categories") ?: obj.optJSONArray("data") ?: return@runCatching
                for (i in 0 until array.length()) { val item = array.optJSONObject(i) ?: continue; val id = item.optString("category_id").ifBlank { item.optString("id") }; val name = item.optString("category_name").ifBlank { item.optString("name") }; if (id.isNotBlank() && name.isNotBlank()) out[id] = name }
            }
        }
        return out
    ...
    private fun readStreamObject(reader: JsonReader, base: String, user: String, pass: String, categories: Map<String, String>): SportsChannel? {
        ...
        reader.beginObject(); while (reader.hasNext()) when (reader.nextName().lowercase()) {
        ...
        val resolvedCategory = categories[categoryId].orEmpty().trim(); if (group.equals("Live TV", true) && resolvedCategory.isNotBlank()) group = resolvedCategory
        val displayName = name.ifBlank { tvgName }.ifBlank { "Channel" }; val url = direct.ifBlank { "$base/live/$user/$pass/$id.$ext" }
        return SportsChannel(id, displayName, group, logo, url, tvgName.ifBlank { displayName }, tvgId, group, provider.ifBlank { "Xtream" })
    }
