...
object StreamClassifier {
    ...
    fun family(channel: SportsChannel): String {
        val base = listOf(channel.name, channel.group, channel.tvgName, channel.tvgId).joinToString(" ").lowercase(Locale.US).replace(Regex("[^a-z0-9]+"), " ").trim()
        return base.split(" ").filter { it.length > 2 && it !in setOf("hd", "fhd", "uhd", "4k", "east", "west", "backup", "feed") }.take(6).joinToString(" ")
    }
}
...
object PlaybackHealth {
    ...
    fun snapshot(url: String): ChannelHealth {
        ...
        return ChannelHealth(s.failures, s.successes, s.lastFailureAt, s.lastSuccessAt, (50 + s.successes * 10 - s.failures * 18).coerceIn(0, MAX_SCORE))
    }
    ...
    fun recordFailure(url: String, failure: PlaybackFailure, now: Long = System.currentTimeMillis()) { if (url.isBlank()) return; val s = state.computeIfAbsent(url) { MutableHealth() }; val weight = when (failure) { PlaybackFailure.DECODER, PlaybackFailure.FORMAT -> 2; else -> 1 }; s.failures = (s.failures + weight).coerceAtMost(20); s.lastFailureAt = now }
}
...
object IntelligentPlayback {
    ...
    fun rank(event: SportsEvent, matches: List<GameSourceMatcher.Match>): List<StreamCandidate> {
        ...
        return ranked.filter { seenFamilies.add(it.family.ifBlank { it.channel.url }) }.take(MAX_ATTEMPTS)
    }
...
object BestSourceMemory {
    ...
    private val memory = object : LinkedHashMap<String, String>(MAX_ENTRIES, 0.75f, true) { override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, String>?): Boolean = size > MAX_ENTRIES }
