...
class SportsChannelDiskStore(context: Context) : SQLiteOpenHelper(context, "usportz_channels.db", null, 6) {
    override fun onConfigure(db: SQLiteDatabase) { super.onConfigure(db); if (!db.isReadOnly) runCatching { db.enableWriteAheadLogging() }; runCatching { db.execSQL("PRAGMA synchronous=NORMAL") } }
    ...
    /** Sports-only projection used by the sports matcher/UI. */
    ...
    fun beginGeneration(sourceKey: String): Long { val generation = System.currentTimeMillis(); val expected = activeCount(sourceKey); writableDatabase.execSQL("INSERT OR REPLACE INTO refresh_state(source_key,generation,imported_count,expected_count,updated_at) VALUES(?,?,?,?,?)", arrayOf(sourceKey, generation, 0, expected, System.currentTimeMillis())); return generation }
    ...
    fun insertBatch(sourceKey: String, generation: Long, batch: List<SportsChannel>) {
        ...
        try { batch.forEach { channel -> statement.clearBindings(); statement.bindLong(1, generation); statement.bindString(2, sourceKey); statement.bindString(3, channel.id); statement.bindString(4, channel.name); statement.bindString(5, channel.group); if (channel.logo.isNullOrBlank()) statement.bindNull(6) else statement.bindString(6, channel.logo); statement.bindString(7, channel.url); statement.bindString(8, channel.tvgName); statement.bindString(9, channel.tvgId); statement.bindString(10, channel.category); statement.bindString(11, channel.provider); statement.bindLong(12, if (SportsNetworkCatalog.isSportsChannel(channel)) 1L else 0L); statement.executeInsert() }; db.setTransactionSuccessful() } finally { statement.close(); db.endTransaction() }
    }
    ...
    fun activate(sourceKey: String, generation: Long, count: Int) {
        ...
        try {
            ...
            if (count <= 0 || stagedCount <= 0) return
            ...
            db.execSQL("DELETE FROM refresh_state WHERE source_key=? AND generation=?", arrayOf(sourceKey, generation))
